...
namespace AWS.MSK.Auth
{
    ...
    /// </summary>
    public class AWSMSKAuthTokenGeneratorAsync
    {
        ...
        private readonly ILogger<AWSMSKAuthTokenGeneratorAsync> _logger;
        ...
        public AWSMSKAuthTokenGeneratorAsync(AmazonSecurityTokenServiceClient? stsClient = null, ILoggerFactory? loggerFactory = null)
        {
            ...
            {
                _logger = loggerFactory.CreateLogger<AWSMSKAuthTokenGeneratorAsync>();
            }
            else
            {
                _logger = NullLoggerFactory.Instance.CreateLogger<AWSMSKAuthTokenGeneratorAsync>();
            }
        }
        ...
        public async Task<(string, long)> GenerateAuthTokenAsync(RegionEndpoint region, bool awsDebugCreds = false)
        {
            ...
            {
                AmazonSecurityTokenServiceClient stsDebugClient = new AmazonSecurityTokenServiceClient(credentials, region);
        ...
        public async Task<(string, long)> GenerateAuthTokenFromCredentialsProviderAsync(Func<AWSCredentials> credentialsProvider, RegionEndpoint region)
        {
            ...
            var signingResult = await Task.Run(() => AWS4PreSignedUrlSigner.SignRequest(request, null, new RequestMetrics(), immutableCredentials.AccessKey,
