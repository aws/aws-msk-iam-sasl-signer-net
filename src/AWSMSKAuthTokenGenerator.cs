...
#region GenerateAuthTokenFromProfile
    ...
    private TimeSpan GetTtl(AWSCredentials credentials)
    {
        ...
        if (credentials is RefreshingAWSCredentials refreshingAwsCredentials)
        {
            ...
            TimeSpan tentativeNewTtl = ttlCredential >= halfPreemptExpiryTime || ttlCredential < TimeSpan.Zero
                ? ttlCredential + preemptExpiryTime
                : ttlCredential + halfPreemptExpiryTime;
        ...
        {
            _logger.LogDebug("Lifetime of token is shorter than default of 15 minutes: {lifetime}s",
